...
import com.android.watchclock.alarms.AlarmStateManager;
import com.android.watchclock.controller.Controller;
import com.android.watchclock.data.DataModel;
import com.android.watchclock.data.Timer;
import com.android.watchclock.data.Weekdays;
import com.android.watchclock.events.Events;
import com.android.watchclock.provider.Alarm;
import com.android.watchclock.provider.AlarmInstance;
import com.android.watchclock.timer.TimerFragment;
import com.android.watchclock.timer.TimerService;
...
import static com.android.watchclock.AlarmSelectionActivity.ACTION_DISMISS;
import static com.android.watchclock.AlarmSelectionActivity.EXTRA_ACTION;
import static com.android.watchclock.AlarmSelectionActivity.EXTRA_ALARMS;
import static com.android.watchclock.provider.AlarmInstance.FIRED_STATE;
import static com.android.watchclock.provider.AlarmInstance.SNOOZE_STATE;
import static com.android.watchclock.uidata.UiDataModel.Tab.ALARMS;
import static com.android.watchclock.uidata.UiDataModel.Tab.TIMERS;
...
public class HandleApiCalls extends Activity {
    ...
    private void handleDismissAlarm(Intent intent) {
        ...
        // Open WatchClock which is now positioned on the alarms tab.
        startActivity(new Intent(mAppContext, WatchClock.class));
    ...
    private void handleSetAlarm(Intent intent) {
        ...
        if (hour == -1) {
            ...
            final Intent createAlarm = Alarm.createIntent(this, WatchClock.class, Alarm.INVALID_ID)
    ...
    private void handleShowAlarms(Intent intent) {
        ...
        // Open WatchClock positioned on the alarms tab.
        UiDataModel.getUiDataModel().setSelectedTab(ALARMS);
        startActivity(new Intent(this, WatchClock.class));
    }

    private void handleShowTimers(Intent intent) {
        Events.sendTimerEvent(R.string.action_show, R.string.label_intent);

        final Intent showTimersIntent = new Intent(this, WatchClock.class);
        ...
        // Open WatchClock positioned on the timers tab.
    ...
    private void handleSetTimer(Intent intent) {
        ...
        if (!intent.hasExtra(AlarmClock.EXTRA_LENGTH)) {
            ...
            // Open WatchClock which is now positioned on the timers tab and show the timer setup.
        ...
        if (!skipUi) {
            ...
            // Open WatchClock which is now positioned on the timers tab.
            startActivity(new Intent(this, WatchClock.class)
    ...
    private void setupInstance(AlarmInstance instance, boolean skipUi) {
        ...
        if (!skipUi) {
            ...
            // Open WatchClock which is now positioned on the alarms tab.
            final Intent showAlarm = Alarm.createIntent(this, WatchClock.class, instance.mAlarmId)
